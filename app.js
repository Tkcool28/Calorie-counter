/* Calorie Quest — FREE mode (v8)
   - Fixes: buttons not working by making init crash-proof
   - Help screen: full-screen modal with Wikipedia answers + meal planner + suggestions
   - Open Food Facts: prefer English; fallback to user query if name is non-Latin
   - Weekly/Monthly/Calendar analytics with charts
*/

const STORAGE_KEY = "calorieQuest_free_v8";
...
function showFatal(err){
  ...
  box.innerHTML = `<b>App error:</b> ${escapeHtml(String(err?.message || err))}<br>
  <span style="opacity:.85">Refresh after clearing site data if needed.</span>`;
}
...
// Non-latin detector (simple + compatible)
function looksNonLatin(s){
  ...
  for (const ch of t){
    ...
    // treat basic latin extended as latin-ish
    if (/[\u00C0-\u024F]/.test(ch)){ letters++; continue; }
    // other letters (Arabic, Cyrillic, etc.)
...
function openHelpScreen(){
  ...
  on($("helpSearchBtn"), "click", runHelpSearch);
  on($("helpOpenBrowserBtn"), "click", () => {
...
function render(){
  ...
  // Goals inputs
  if ($("g_cal")) $("g_cal").value = goals.calories;
  if ($("g_p")) $("g_p").value = goals.protein;
  if ($("g_c")) $("g_c").value = goals.carbs;
  if ($("g_f")) $("g_f").value = goals.fat;
...
function renderAnalytics(){
  const tab7 = $("tab7"), tab30 = $("tab30"), tabCal = $("tabCal");
