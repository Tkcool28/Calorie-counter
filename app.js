/* Calorie Quest — 4-file root-only PWA (FREE mode)
   - Offline DB + Open Food Facts (English preferred)
   - Daily goals + progress bars + motivation
   - Weekly/monthly analytics + charts
   - Help button opens full-screen Help Screen:
       * Ask a nutrition question
       * Free web answers via Wikipedia summaries + links
       * Open full web search via DuckDuckGo
       * Smart suggestions + meal planner
*/

const STORAGE_KEY = "calorieQuest_free_v6";
...
// ---------------- Open Food Facts (English preferred) ----------------
async function searchOpenFoodFacts(query){
  const q = (query || "").trim();
  if (!q) return null;

  // Force English UI + ask for product_name_en
  const url =
...
function openHelpScreen(){
  ...
  helpModalEl.innerHTML = `
    <div class="helpInner">
      ...
      <div class="helpBody">
        <div class="field">
          <label>Ask a nutrition question (free search)</label>
          <input id="helpQuestion" placeholder="e.g., High protein snack under 250 calories?" />
          <div class="hint">Uses Wikipedia summaries + links. For deeper research, open full web search.</div>
        </div>
  ...
  $("helpSearchBtn").onclick = runHelpSearch;
...
function renderHelpMealPlan(){
  ...
  mealPlanCached = generateMealPlan(goals);
  $("helpPlanOut").innerHTML = Object.entries(mealPlanCached).map(([meal, items]) => `
...
// Free web answers using Wikipedia OpenSearch + summaries (works client-side)
