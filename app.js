/* Calorie Quest — 4-file root-only PWA
   - Offline DB + Open Food Facts (forced English)
   - Daily goals + progress bars + motivation
   - Weekly / monthly analytics
   - Full-screen Help Screen:
       * Nutrition-only AI via Cloudflare Worker (OpenAI)
       * Wikipedia “web search” answers
*/
...
// 1) After you deploy the Cloudflare Worker, paste the Worker URL here.
// Example: "https://calorie-quest-ai.your-subdomain.workers.dev"
const WORKER_URL = ""; // <-- REQUIRED for AI answers (leave "" to disable AI)

// 2) If you host the worker on a custom route like /ask, keep as-is
const WORKER_ASK_PATH = "/ask";

///////////////////////

const STORAGE_KEY = "calorieQuest_v5_root";
...
function motivation(ratio, over){
  if (over){
    return pick([
...
// -------------------- Open Food Facts (FORCE ENGLISH) --------------------
async function searchOpenFoodFacts(query){
  const q = (query || "").trim();
  if (!q) return null;

  // Force English UI + prefer product_name_en
  const url =
  ...
  const pname = product.product_name_en || product.product_name || q;
  const name = [pname, product.brands].filter(Boolean).join(" — ") || q;
...
function ensureDay(state, key){
  if(!state.days[key]){
    state.days[key] = { goals: { ...DEFAULT_GOALS }, entries: [] };
  }
...
function barHTML(label, badge, current, goal, unit){
  const g = Math.max(goal, 1e-6);
  const ratio = current / g;
  const over = ratio > 1;
  ...
  return `
    <div class="bar">
      <div class="barTop">
        ...
        <div class="hint">${round0(current)}/${round0(goal)} ${unit} (${Math.min(ratio*100,999).toFixed(0)}%)</div>
      </div>
      <div class="track"><div class="fill" style="width:${width}"></div></div>
      <div class="mot">${motivation(ratio, over)}</div>
    </div>
  `;
}
...
function rangeKeys(endKey, daysBack){
  ...
  for(let i=daysBack-1;i>=0;i--){
    keys.push(keyForDate(addDays(end, -i)));
  }
  return keys;
}

function summarizeRange(state, keys){
...
function openHelpScreen(){
  ...
  helpModalEl.innerHTML = `
    <div class="helpInner">
      <div class="helpTop">
        <div class="helpTitle">Help & Answers 🤖</div>
      ...
      <div class="helpBody">
        <div class="field">
          ...
          <div class="hint">Nutrition only: calories, macros, meal ideas, food substitutions, etc.</div>
        </div>

        <div class="row" style="margin-top:10px;">
          <button class="btn primary grow" id="helpAiBtn">AI answer ✔️</button>
          <button class="btn ghost grow" id="helpWebBtn">Web search (Wikipedia)</button>
          <button class="btn ghost grow" id="helpOpenBrowserBtn">Open full web search</button>
        </div>

        <div class="helpSplit">
          <div>
            <div class="helpBoxTitle">AI Answer</div>
            <div id="helpAiOut" class="subcard"><div class="hint">Tap “AI answer”.</div></div>
          </div>
          <div>
            <div class="helpBoxTitle">Web Results</div>
            <div id="helpWebOut" class="subcard"><div class="hint">Tap “Web search”.</div></div>
          </div>
        </div>

        <div class="hr"></div>
        <div class="helpBoxTitle">Smart “what to eat next” (based on your remaining goals)</div>
  ...
  $("helpCloseBtn").onclick = closeHelpScreen;
  $("helpAiBtn").onclick = runHelpAI;
  $("helpWebBtn").onclick = runHelpWeb;
  $("helpOpenBrowserBtn").onclick = () => {
    const q = (($("helpQuestion").value || "").trim());
    if (!q) return;
    window.open(`https://duckduckgo.com/?q=${encodeURIComponent(q)}`, "_blank");
  };

  $("helpPlanBtn").onclick = () => renderHelpMealPlan();
  $("helpPlanClearBtn").onclick = () => { $("helpPlanOut").innerHTML = `<div class="hint">Tap “Generate meal plan”.</div>`; };
...
function renderHelpSuggestions(){
  ...
  const suggestions = suggestFoods(remaining, "balanced");
...
async function runHelpAI(){
  ...
  if (!q){
    out.innerHTML = `<div class="hint">Ask a nutrition question first.</div>`;
    return;
  }

  if (!WORKER_URL){
    out.innerHTML = `<div class="hint">
      AI is not configured yet.<br>
      Deploy the Cloudflare Worker, then paste its URL into <code>WORKER_URL</code> in <b>app.js</b>.
    </div>`;
    return;
  }
  ...
  try{
    const res = await fetch(WORKER_URL.replace(/\/+$/,"") + WORKER_ASK_PATH, {
      method: "POST",
    ...
    if (!res.ok){
      ...
      out.innerHTML = `<div class="hint">AI request failed (${res.status}). ${escapeHtml(txt).slice(0,160)}</div>`;
      return;
    }

    const data = await res.json();
    const answer = data?.answer || "No answer returned.";
    out.innerHTML = `<div class="hint">${escapeHtml(answer).replaceAll("\n","<br>")}</div>`;
  }catch{
    out.innerHTML = `<div class="hint">AI request failed (network/CORS). Make sure the Worker is deployed and public.</div>`;
  }
}

// Wikipedia search + summary fetch (client-side friendly)
async function runHelpWeb(){
  const q = (($("helpQuestion").value || "").trim());
  const out = $("helpWebOut");
  ...
  try{
    const sUrl = `https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(q)}&limit=5&namespace=0&format=json&origin=*`;
    const sRes = await fetch(sUrl);
    const sData = await sRes.json();
    const titles = (sData?.[1] || []);

    if (!titles.length){
      out.innerHTML = `<div class="hint">No Wikipedia matches. Try “Open full web search”.</div>`;
      return;
    }
    ...
    out.innerHTML = `<div class="hint">Search failed. Try “Open full web search”.</div>`;
  }
}
...
function render(){
  ...
  $("btnLog").onclick = doLog;

  // Help button now opens Help Screen
  $("btnHelp").onclick = openHelpScreen;
}
...
async function doSearch(){
  ...
  try{
    ...
    if (m && m.score >= 0.20){
      const perServing = scalePer100(OFFLINE_DB[m.name], grams);
...
function renderAnalytics(){
  ...
  $("tab7").onclick = () => { analyticsTab = "7"; persistUI(); render(); };
  $("tab30").onclick = () => { analyticsTab = "30"; persistUI(); render(); };
  $("tabCal").onclick = () => { analyticsTab = "cal"; persistUI(); render(); };

  $("monthPicker").classList.toggle("hidden", analyticsTab !== "cal");

  const keys7 = rangeKeys(todayKey, 7);
  const keys30 = rangeKeys(todayKey, 30);
  const week = summarizeRange(state, keys7);
  const month30 = summarizeRange(state, keys30);
  ...
  $("selMonth").onchange = (e) => { selectedMonth = e.target.value; persistUI(); render(); };

  const [selY, selM] = selectedMonth.split("-").map(Number);
  const keysCal = monthKeys(selY, selM-1);
  const cal = summarizeRange(state, keysCal);
...
function init(){
  ...
  // manual toggle should re-render panels
